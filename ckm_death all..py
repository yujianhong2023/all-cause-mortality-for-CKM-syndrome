...
with col_right:
    ...
    if predict_clicked:
        try:
            ...
            # Risk Stratification Update (Thresholds: 7.5% and 20%)
            ...
            # 3. Clinical Recommendations based on Risk Level
            st.markdown("### 🩺 Clinical Recommendations")

            if risk < 0.075:
                st.success("""
                **📉 Low Risk Group (Predicted Probability < 7.5%)**  
                **Core Strategy:** Primary prevention, maintaining optimal cardiovascular health.

                **Clinical Guidance:**
                - **Continuous Monitoring:** Routine screening and evaluation of metabolic risk factors and renal function are recommended for all adults.
                - **Healthy Lifestyle:** Emphasize maintaining normal weight, blood glucose, blood pressure, and lipids through lifestyle interventions (e.g., proper diet, regular exercise).
                - **Assess Social Determinants of Health (SDOH):** Screen and intervene for adverse SDOH, which is a core component of holistic care.
                """)
            elif risk < 0.20:
                st.warning("""
                **📊 Moderate Risk Group (Predicted Probability 7.5% - < 20%)**  
                **Core Strategy:** Initiate pharmacological intervention, prioritize medications with cardiorenal protective effects.

                **Clinical Guidance:**
                - **Initiate Cardiorenal Protective Medications:** For patients with type 2 diabetes or high cardiovascular risk, Sodium-Glucose Cotransporter-2 inhibitors (SGLT2i) or Glucagon-Like Peptide-1 Receptor Agonists (GLP-1 RA) should be considered.
                - **Intensive Lifestyle Intervention:** Weight loss medications can be combined with lifestyle interventions.
                - **Assess CKM-related Comorbidities:** Comprehensively evaluate for the presence of pre-heart failure, Metabolic Dysfunction-Associated Steatotic Liver Disease (MASLD), Obstructive Sleep Apnea (OSA), etc.
                """)
            else:
                st.error("""
                **⚠️ High Risk Group (Predicted Probability ≥ 20%)**  
                **Core Strategy:** Intensify multidisciplinary comprehensive management. This risk level is one of the criteria defining Stage 3 CKM syndrome.

                **Clinical Guidance:**
                - **Initiate Multidisciplinary Collaboration:** Requires collaboration among cardiology, nephrology, endocrinology, and other Multidisciplinary Teams (MDT).
                ...
                - **Close Follow-up:** It is recommended that patients with CKM syndrome stages 2-4 be followed up at least 4 times a year.
                """)
            ...
            # SHAP Force Plot (Fixed Javascript Issue)
            ...
            # Inject SHAP's required JS bundle manually using shap.getjs()
            shap_html = f"<head>{shap.getjs()}</head><body>{shap_force.html()}</body>"
